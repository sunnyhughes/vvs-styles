...
**Done-when:**
- [x] `/about` and `/faq` pages exist with real copy (placeholder copy OK if real not ready, but flag). _Built 2026-07-08, finished 2026-07-09. Both placeholders are now closed: the `[FOUNDER: …]` bracket is replaced by Sunshine's real story (rendered as a signed, unnamed `<blockquote>` so first-person testimony doesn't collide with the page's "we" voice or the brand's anonymity), and `CONTACT_EMAIL` is now the real `esohcreationsllc@gmail.com`. Added a "Who these shirts are for" section — the founder frames recovery broadly (abuse, misuse, mistakes; "survivors"), not substance-only. A test now guards against any `[PLACEHOLDER:` bracket reaching the About page. 87/87 green; typecheck clean. **Not yet committed/deployed.**_
- [ ] axe-core audit reports zero violations across all routes. _Needs an axe dependency (jest-axe/vitest-axe) — ask before adding per CLAUDE.md._
- [ ] Lighthouse mobile score ≥ 90 in all four categories.
- [ ] README has public URL, demo video, PRD video, launch checklist.
- [x] Social links in the footer. _Instagram + Facebook verified live 2026-07-09; TikTok wired but unverifiable programmatically (TikTok blocks fetches) — click once after deploy. All three are Esoh Creations handles. Brand glyphs are inline SVG: DESIGN.md specifies Heroicons, which has no brand marks, and adding a dep needs a nod per CLAUDE.md._
- [ ] Soft launch posted to social. _Blocked on intro-promo note + Stripe live-key roll._
...
| 2026-07-08 | Phase 5 | Built About + FAQ + rebrand (not yet deployed) | Wrote `/about` (founder story) and `/faq` (Headless UI `Disclosure` accordion) with copy extracted to `content/about.ts` + `content/faq.ts`; routed both in `App.tsx`. Locked brand canon: **vv-styles** ("vv" = Very Vocal; dropped the "s"=speech) as the storefront, under **Esoh Creations LLC** umbrella (footer + About); final domain vv-styles.com (DNS unwired). Rebrand vvs-styles→vv-styles in Nav/Footer/`index.html`/confirmation email; left infra (worker/D1 names, `vvs-cart` key, dropship UA + workers.dev URL) untouched. Launch decisions locked: **free shipping baked into $24.99**, **returns = defects/wrong-item only** (made-to-order/personalized final sale), payments FAQ lists Stripe methods live + "PayPal/Venmo coming soon" (PayPal needs Phase 3.5 — Stripe can't do PayPal for US/USD; Business account started 2026-07-08). 80/80 tests green (adds About + FAQ page tests), typecheck clean. Still open before soft launch: `[FOUNDER: …]` bracket, `CONTACT_EMAIL`, socials URLs, intro-promo note, axe-core a11y test (needs a dep), Lighthouse run, README checklist, Stripe live roll, deploy. |
| 2026-07-09 | Phase 5 | Closed both copy placeholders; wired socials + custom domain | Founder story captured from Sunshine directly and rendered as a signed-but-unnamed `<blockquote>` on `/about`; added a "Who these shirts are for" section because her framing of recovery is **broader than substance use** (abuse, misuse, mistakes — the reader is a "survivor" proud of any journey), which the old copy didn't carry. `CONTACT_EMAIL` → `esohcreationsllc@gmail.com` (one shared Esoh mailbox across ventures for now; a branded `hello@vv-styles.com` can be added later via free Cloudflare Email Routing forwarding). Footer gained Instagram/Facebook/TikTok links (inline SVG brand glyphs, `aria-label` per link, `rel=noopener`) — IG + FB verified to resolve, TikTok not fetchable. Custom domain wired in `wrangler.toml` as two `[[routes]]` (`vv-styles.com` + `www.vv-styles.com`, `custom_domain = true`) — apex does **not** cover www, they match exactly. **Gotcha:** `@cloudflare/vite-plugin` redirects wrangler to `dist/vvs_styles/wrangler.json`, so `wrangler.toml` route edits do nothing until `npm run build` regenerates it — verified routes land post-build. 87/87 green; typecheck clean. Nothing deployed. Open: intro-promo note, Stripe live-key roll, axe dep, Lighthouse, README, and the origin phrase **"Proud to be Recovering" is not in the catalog** (closest is `recovering-out-loud`). |
